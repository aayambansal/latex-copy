...
async function uploadProject(req, res, next) {
  ...
  let userId = SessionManager.getLoggedInUserId(req.session)
  
  // Public access - create anonymous user if needed
  if (Settings.allowPublicAccess && !userId) {
    const crypto = await import('node:crypto')
    const UserCreator = (await import('../User/UserCreator.mjs')).default
    ...
    try {
      ...
      // Set user in session
      SessionManager.setSessionUser(req.session, user)
      userId = user._id
      logger.info({ userId }, 'Created anonymous user for project upload')
    } catch (error) {
      logger.error({ err: error }, 'Failed to create anonymous user for upload')
