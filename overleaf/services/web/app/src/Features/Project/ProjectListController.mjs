...
async function projectListPage(req, res, next) {
  ...
  // Public access - handle anonymous users
  if (Settings.allowPublicAccess && !userId) {
    // Anonymous user - show empty project list
    // Create minimal user object for anonymous access
    const anonymousUser = {
      signUpDate: new Date(),
      ace: {
        mode: 'none',
        ...
        mathPreview: false,
        breadcrumbs: false,
        referencesSearchMode: 'all',
        enableNewEditor: true,
        enableNewEditorStageFour: true,
        darkModePdf: false,
      },
    }
    const userSettings = await UserSettingsHelper.buildUserSettings(req, res, anonymousUser)
    res.render('project/list-react', {
      title: 'your_projects',
      usersBestSubscription: undefined,
      notifications: [],
      ...
      userEmails: [],
      userSettings,
      reconfirmedViaSAML: false,
      allInReconfirmNotificationPeriods: false,
      survey: null,
      tags: [],
      portalTemplates: [],
      prefetchedProjectsBlob: { projects: [], totalSize: 0 },
      showGroupsAndEnterpriseBanner: false,
      groupsAndEnterpriseBannerVariant: null,
      showUSGovBanner: false,
      usGovBannerVariant: null,
      showLATAMBanner: false,
      recommendedCurrency: null,
      showInrGeoBanner: false,
      projectDashboardReact: true,
      groupSsoSetupSuccess: false,
      joinedGroupName: null,
      viaDomainCapture: false,
      groupSubscriptionsPendingEnrollment: [],
      hasIndividualPaidSubscription: false,
      userRestrictions: [],
  ...
  if (user == null) {
    // Public access - allow anonymous access
...
async function getProjectsJson(req, res) {
  ...
  // Public access - return empty projects if no user
  if (Settings.allowPublicAccess && !userId) {
    return res.json({
      projects: [],
      totalSize: 0,
