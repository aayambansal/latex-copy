...
const _ProjectController = {
  ...
  async newProject(req, res) {
    let currentUser = SessionManager.getSessionUser(req.session)
    
    // Public access - create anonymous user if needed
    if (Settings.allowPublicAccess && !currentUser) {
      // Generate a unique anonymous email
      const anonymousId = crypto.randomBytes(16).toString('hex')
      const anonymousEmail = `anonymous-${anonymousId}@inkvell.local`
      
      try {
        // Create anonymous user
        currentUser = await UserCreator.promises.createNewUser({
          ...
          confirmedAt: new Date(), // Auto-confirm anonymous users
        })
        
        // Log them in automatically
        SessionManager.setSessionUser(req.session, currentUser)
        logger.info({ userId: currentUser._id }, 'Created anonymous user for project creation')
      } catch (error) {
        logger.error({ err: error }, 'Failed to create anonymous user')
  ...
  async loadEditor(req, res, next) {
    ...
    try {
      ...
      // Public access - allow anyone to access projects
      if (!Settings.allowPublicAccess && (privilegeLevel == null || privilegeLevel === PrivilegeLevels.NONE)) {
